...
import { startLoginServer, setLoginQR, setLoginStatus } from './lib/loginServer.js';
...
// 'qr' (default) = tampilkan QR code di halaman web (lib/loginServer.js).
// 'pairing'       = kode 8-digit berbasis nomor HP, muncul di console.
// ENV LOGIN_METHOD bisa override tanpa edit setting.js.
const LOGIN_METHOD = String(process.env.LOGIN_METHOD || settings.loginMethod || 'qr').toLowerCase() === 'pairing'
    ? 'pairing'
    : 'qr';
...
async function startBot() {
    ...
    if (LOGIN_METHOD === 'pairing' && !state.creds.registered && !isNomorValid(NOMOR_HP)) {
...
if (qr && LOGIN_METHOD === 'qr') {
    setLoginQR(qr);
    log.info('📲 QR code baru tersedia — buka halaman web bot ini (URL Render kamu) untuk scan.');
}
...
if (connection === 'open') {
    ...
    log.connect(
        ...
        if (connection === 'close') {
            ...
            if (code === DisconnectReason.loggedOut) {
                ...
                setLoginStatus('error', 'Logout dari WhatsApp — perlu scan/pairing ulang.');
                clearSession();
                process.exit(0);
    ...
    if (LOGIN_METHOD === 'pairing' && !state.creds.registered) {
        ...
        setLoginStatus('qr', `Pairing code: ${formatted}`);
        console.log('\n' + '╔' + '═'.repeat(44) + '╗');
...
// ── Halaman web login/QR + health check untuk Render ─────────────────
// Dinyalakan SEKALI di sini, sebelum startBot() — supaya port-nya sudah
// ke-bind dan health check Render langsung lolos, tidak perlu nunggu
// WhatsApp sempat konek dulu. Lihat lib/loginServer.js untuk detailnya.
startLoginServer();
